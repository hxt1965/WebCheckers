 ...
 * @author <a href='mailto:pgw5563@g.rit.edu'>Paul Woods</a>
 */
public class WebServer {
  ...
  /**
   * The URL pattern to request start game page
   */
  private static final String START_GAME = "/startgame";

  /**
   * The URL pattern to request to validate a move.
   */
  private static final String VALIDATE_URL = "/validateMove";

  /**
   * The URL pattern to request to end your turn.
   */
  private static final String SUBMIT_URL = "/submitMove";

  /**
   * The URL pattern to request to back up a move.
   */
  private static final String BACKUP_URL = "/backupMove";

  /**
   * The URL pattern to request to end the game with your defeat.
   */
  private static final String RESIGN_URL = "/resignGame";

  /**
   * The URL pattern to check to see if it's your turn.
   */
  private static final String CHECK_URL = "/checkTurn";
   ...
   */
  public WebServer(final GameCenter gameCenter,final TemplateEngine templateEngine, final Gson gson, final PlayerLobby playerLobby) {
    // validation
  ...
  public void initialize() {
    ...
    get(HOME_URL, new GetHomeRoute(gameCenter, templateEngine, playerLobby));

    // Shows sign-in page
    get(SIGN_IN, new GetSigninRoute(templateEngine));

    // Post a username to use.
    post(SIGN_IN, new PostSigninRoute(gameCenter, templateEngine, playerLobby));

    // Signs out
    post(SIGN_OUT, new PostSignOutRoute(gameCenter, playerLobby));

    // Post Home to start a game
    post(HOME_URL, new PostHomeRoute(gameCenter, playerLobby));

    // Game on
    get(GAME_URL, new GetGameRoute(gameCenter, templateEngine));

    // Posts gameID and Move to current game.
    post(VALIDATE_URL, new PostValidateMove(gameCenter, templateEngine));

    // Posts gameID access game to submit turn.
    post(SUBMIT_URL, new PostSubmitTurn(gameCenter, templateEngine));

    // Posts gameID to access game and backup a move.
    post(BACKUP_URL, new PostBackUpMove(gameCenter, templateEngine));

    // Posts gameID to access game and resign the game.
    post(RESIGN_URL, new PostResignGame(gameCenter, templateEngine));

    // Posts a message with an optional game ID to tell player if it's their turn.
    post(CHECK_URL, new PostCheckTurn(gameCenter, templateEngine));
